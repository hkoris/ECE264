...
struct Image * loadImage(const char* filename)
{
	...
	if(imgHead.width == 0 || imgHead.height == 0){
		printf("\nImage height and/or width is zero\n");
		fclose(fil);
		return NULL;
	}
	if(imgHead.magic_bits != ECE264_IMAGE_MAGIC_BITS){
		printf("\nmagic bits is not ECE264_IMAGE_MAGIC_BITS\n");
		fclose(fil);
		return NULL;
	}
	...
	if((img->comment)[imgHead.comment_len-1] != NULL_TERMINATOR){
		printf("\nthe comment is not NULL terminated\n");
		freeImage(img);
		fclose(fil);
		return NULL;
	}
...
void linearNormalization(struct Image * image)
{
	if(image != NULL){
		...
		for(i = 0;i < size;i++){
			pixels[i] = (pixels[i] - min) * MAX_INTENSITY / (max - min);
		}
	}
}
