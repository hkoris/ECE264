...
SparseNode* SparseArray_copy_helper(SparseNode *, SparseNode*);
SparseNode* SparseArray_remove_helper(SparseNode*);
SparseNode* SparseArray_merge_helper(SparseNode*, SparseNode*);
...
SparseNode * SparseArray_remove ( SparseNode * array, int index )
{
	...
	if(array->index == index){
		...
		if(array->left == NULL){
			SparseNode* replacement = array->right;
			free(array);
			return replacement;
		}
		if(array->right == NULL){
			SparseNode* replacement = array->left;
			free(array);
			return replacement;
		}
		SparseNode* replacement = SparseArray_remove_helper(array->right);
		array->index = replacement->index;
...
SparseNode* SparseArray_remove_helper(SparseNode* array){
	if(array->left == NULL){
		return array;
	}
	return SparseArray_remove_helper(array->left);
}
...
SparseNode * SparseArray_copy(SparseNode * array)
{
  ...
  return SparseArray_copy_helper(array, copy);
}

SparseNode * SparseArray_copy_helper(SparseNode * array, SparseNode* copy){
	if(array == NULL){
		return NULL;
	}
	copy = SparseArray_insert(copy, array->index, array->value);

	SparseArray_copy_helper(array->left, copy);
	SparseArray_copy_helper(array->right, copy);
	return copy;
}
...
SparseNode * SparseArray_merge(SparseNode * array_1, SparseNode * array_2)
{
	...
	copy = SparseArray_merge_helper(copy, array_2);
	
  return copy;
}

SparseNode* SparseArray_merge_helper(SparseNode* copy, SparseNode* array_2){
	if(array_2 == NULL){
		return copy;
	}

	SparseNode* node_1 = SparseArray_getNode(copy, array_2->index);
	if(node_1 == NULL){
		copy = SparseArray_insert(copy, array_2->index, array_2->value);
	}
	else{
		node_1->value += array_2->value;
		if(node_1->value == 0){
			copy = SparseArray_remove(copy, node_1->index);
		}
	}

	copy = SparseArray_merge_helper(copy, array_2->left);
	copy = SparseArray_merge_helper(copy, array_2->right);
	return copy;
}
